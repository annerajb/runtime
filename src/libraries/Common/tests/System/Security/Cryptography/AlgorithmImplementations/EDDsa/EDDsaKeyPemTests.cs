...
namespace System.Security.Cryptography.Tests
{
    ...
    public abstract class EDDsaKeyPemTests : EDDsa
    {
        ...
        protected EDDsa CreateKey() => EDDsa.Create();

        [Fact]
        public void ImportFromPem_NoPem()
        {
            using (EDDsa key = CreateKey())
            {
        ...
        [Fact]
        public void ImportFromPem_EDPrivateKey_Simple()
        {
            using (EDDsa key = CreateKey())
            {
                key.ImportFromPem(@"
-----BEGIN PRIVATE KEY-----
BCAQGoF0bN67Q4lAO4yOpiJadOD9ggou+4sHThzhQP35Gg==
-----END PRIVATE KEY-----");
                EDDsaParameters ecParameters = key.ExportParameters(true);
                EDDsaParameters expected = EDDsaTestData.GetNistP256ReferenceKey();
                EDDsaTestsBase.AssertEqual(expected, ecParameters);
            }
        }

        [Fact]
        public void ImportFromPem_EDPrivateKey_IgnoresUnrelatedAlgorithm()
        {
            using (EDDsa key = CreateKey())
            {
                key.ImportFromPem(@"
...
-----END RSA PRIVATE KEY-----
-----BEGIN PRIVATE KEY-----
MC4CAQAwBQYDK2VwBCIEIBAagXRs3rtDiUA7jI6mIlp04P2CCi77iwdOHOFA/fka
-----END PRIVATE KEY-----");
                EDDsaParameters ecParameters = key.ExportParameters(true);
                EDDsaParameters expected = EDDsaTestData.GetNistP256ReferenceKey();
                EDDsaTestsBase.AssertEqual(expected, ecParameters);
...
// -----END PUBLIC KEY-----");
//                 EDDsaParameters ecParameters = key.ExportParameters(false);
//                 EDDsaParameters expected = EDDsaTestData.GetNistP256ReferenceKey();
//                 EDDsaTestsBase.ComparePublicKey(expected.Q, ecParameters.Q, isEqual: true);
//             }
//         }
...
// VQQGEwJVUzETMBEGA1UEDAwKV2FzaGluZ3RvbjEQMA4GA1UEBwwHUmVkbW9uZDEY
// MBYGA1UEDgwPTWljcm9zb2Z0IENvcnAuMSAwHgYDVQQLDBcuTkVUIEZyYW1ld29y
...
// Y3Jvc29mdCBDb3JwLjEgMB4GA1UEDwwXLk5FVCBGcmFtZXdvcmsgKENvcmVGeCkx
