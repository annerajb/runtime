...
namespace System.Security.Cryptography.EdDsa.Tests
{
    [SkipOnPlatform(TestPlatforms.Browser, "Not supported on Browser")]
    public static class EDDsaKeyFileTests
    {
//         public static bool Supports384BitPrivateKeyAndRC2 { get; } = EDDsaFactory.Supports384PrivateKey && RC2Factory.IsSupported;
//         public static bool SupportsLargeExponent { get; } = EDDsaFactory.SupportsLargeExponent;
...
//         {
//             EDDsa EDDsa = importKey ? EDDsaFactory.Create(TestData.EDDsa2048Params) : EDDsaFactory.Create(1024);
//             byte[] pkcs1Public;
//             byte[] pkcs1Private;
//             byte[] pkcs8Private;
...
//             using (EDDsa)
//             {
//                 pkcs1Public = EDDsa.ExportEDDsaPublicKey();
//                 pkcs1Private = EDDsa.ExportEDDsaPrivateKey();
//                 pkcs8Private = EDDsa.ExportPkcs8PrivateKey();
//                 pkcs8EncryptedPrivate = EDDsa.ExportEncryptedPkcs8PrivateKey(pwStr, pbeParameters);
//                 subjectPublicKeyInfo = EDDsa.ExportSubjectPublicKeyInfo();
//             }

//             Assert.Throws<ObjectDisposedException>(() => EDDsa.ImportEDDsaPublicKey(pkcs1Public, out _));
//             Assert.Throws<ObjectDisposedException>(() => EDDsa.ImportEDDsaPrivateKey(pkcs1Private, out _));
//             Assert.Throws<ObjectDisposedException>(() => EDDsa.ImportPkcs8PrivateKey(pkcs8Private, out _));
//             Assert.Throws<ObjectDisposedException>(() => EDDsa.ImportEncryptedPkcs8PrivateKey(pwStr, pkcs8EncryptedPrivate, out _));
//             Assert.Throws<ObjectDisposedException>(() => EDDsa.ImportEncryptedPkcs8PrivateKey(pwBytes, pkcs8EncryptedPrivate, out _));
//             Assert.Throws<ObjectDisposedException>(() => EDDsa.ImportSubjectPublicKeyInfo(subjectPublicKeyInfo, out _));

//             Assert.Throws<ObjectDisposedException>(() => EDDsa.ExportEDDsaPublicKey());
//             Assert.Throws<ObjectDisposedException>(() => EDDsa.TryExportEDDsaPublicKey(pkcs1Public, out _));
//             Assert.Throws<ObjectDisposedException>(() => EDDsa.ExportEDDsaPrivateKey());
//             Assert.Throws<ObjectDisposedException>(() => EDDsa.TryExportEDDsaPrivateKey(pkcs1Private, out _));
//             Assert.Throws<ObjectDisposedException>(() => EDDsa.ExportPkcs8PrivateKey());
//             Assert.Throws<ObjectDisposedException>(() => EDDsa.TryExportPkcs8PrivateKey(pkcs8Private, out _));
//             Assert.Throws<ObjectDisposedException>(() => EDDsa.ExportEncryptedPkcs8PrivateKey(pwStr, pbeParameters));
//             Assert.Throws<ObjectDisposedException>(() => EDDsa.TryExportEncryptedPkcs8PrivateKey(pwStr, pbeParameters, pkcs8EncryptedPrivate, out _));
//             Assert.Throws<ObjectDisposedException>(() => EDDsa.ExportEncryptedPkcs8PrivateKey(pwBytes, pbeParameters));
//             Assert.Throws<ObjectDisposedException>(() => EDDsa.TryExportEncryptedPkcs8PrivateKey(pwBytes, pbeParameters, pkcs8EncryptedPrivate, out _));
//             Assert.Throws<ObjectDisposedException>(() => EDDsa.ExportSubjectPublicKeyInfo());
//             Assert.Throws<ObjectDisposedException>(() => EDDsa.TryExportSubjectPublicKeyInfo(subjectPublicKeyInfo, out _));
...
//             Assert.Throws<ObjectDisposedException>(() => EDDsa.ImportEncryptedPkcs8PrivateKey("", pkcs8EncryptedPrivate, out _));
//             Assert.Throws<ObjectDisposedException>(() => EDDsa.ImportEncryptedPkcs8PrivateKey(pwBytes, pkcs8EncryptedPrivate, out _));
//         }
...
//                 TestData.EDDsaBigExponentParams);
//         }
...
//         [ConditionalFact(typeof(EDDsaImportExportTests), nameof(EDDsaImportExportTests.Supports16384))]
...
//         [ConditionalFact(typeof(EDDsaImportExportTests), nameof(EDDsaImportExportTests.Supports16384))]
//         public static void ReadWriteEDDsa16384SubjectPublicKeyInfo()
//         {
//             ReadWriteBase64SubjectPublicKeyInfo(
//                 @"
...
//         [Fact]
//         public static void ReadEncryptedEDDsa1032()
//         {
//             // PBES2: PBKDF2 + aes192
//             const string base64 = @"
...
//                     10583),
//                 TestData.EDDsa1032Parameters);
//         }

//         [ConditionalFact(typeof(EDDsaImportExportTests), nameof(EDDsaImportExportTests.Supports16384))]
//         public static void ReadEncryptedEDDsa16384()
//         {
...
//         public static void ReadPbes1Rc2EncryptedEDDsa384()
//         {
...
//                     2050),
//                 TestData.EDDsa384Parameters);
//         }

        [Fact]
        public static void EDDsaPrivatePkcs8Simple()
        {
            using (EDDsa key = EDDsa.Create())
            {
                int bytesRead = -1;
                //byte[] EDDsaPriv = key.ExportEDDsaPrivateKey();
                ReadOnlySpan<byte> privpkcs8 = Convert.FromBase64String("MC4CAQAwBQYDK2VwBCIEIH930MEL21fT8kPwzjOiHdqWggUShEUcf6SgfQvpGg/E");
                key.ImportPkcs8PrivateKey(privpkcs8, out bytesRead);
                ...
                // //     () => key.ImportEDDsaPublicKey(spki, out bytesRead));
                ...
                // // Assert.ThrowsAny<CryptographicException>(
                // //     () => key.ImportEDDsaPublicKey(pkcs8, out bytesRead));

                // Assert.Equal(-1, bytesRead);

                // ReadOnlySpan<byte> passwordBytes = EDDsaPriv.AsSpan(0, 15);
                ...
                // Assert.ThrowsAny<CryptographicException>(
                //     () => key.ImportEDDsaPublicKey(encryptedPkcs8, out bytesRead));

                // Assert.Equal(-1, bytesRead);
            }
        }
//         [Fact]
//         public static void NoFuzzyEDDsaPublicKey()
//         {
//             using (EDDsa key = EDDsaFactory.Create())
//             {
//                 int bytesRead = -1;
//                 byte[] EDDsaPriv = key.ExportEDDsaPrivateKey();

//                 Assert.ThrowsAny<CryptographicException>(
//                     () => key.ImportEDDsaPublicKey(EDDsaPriv, out bytesRead));
...
//                 Assert.ThrowsAny<CryptographicException>(
//                     () => key.ImportSubjectPublicKeyInfo(EDDsaPriv, out bytesRead));

//                 Assert.Equal(-1, bytesRead);

//                 byte[] EDDsaPub = key.ExportEDDsaPrivateKey();

//                 Assert.ThrowsAny<CryptographicException>(
//                     () => key.ImportSubjectPublicKeyInfo(EDDsaPub, out bytesRead));
...
//         [Fact]
//         public static void NoFuzzyEDDsaPrivateKey()
//         {
//             using (EDDsa key = EDDsaFactory.Create())
//             {
//                 int bytesRead = -1;
//                 byte[] EDDsaPub = key.ExportEDDsaPublicKey();

//                 Assert.ThrowsAny<CryptographicException>(
//                     () => key.ImportEDDsaPrivateKey(EDDsaPub, out bytesRead));
...
//                 Assert.ThrowsAny<CryptographicException>(
//                     () => key.ImportEDDsaPrivateKey(spki, out bytesRead));
...
//                 Assert.ThrowsAny<CryptographicException>(
//                     () => key.ImportEDDsaPrivateKey(pkcs8, out bytesRead));
...
//                 Assert.ThrowsAny<CryptographicException>(
//                     () => key.ImportEDDsaPrivateKey(encryptedPkcs8, out bytesRead));
...
//                 Assert.ThrowsAny<CryptographicException>(
//                     () => key.ImportPkcs8PrivateKey(EDDsaPub, out bytesRead));
...
//                 Assert.ThrowsAny<CryptographicException>(
//                     () => key.ImportPkcs8PrivateKey(EDDsaPriv, out bytesRead));
...
//                 Assert.ThrowsAny<CryptographicException>(
//                     () => key.ImportEncryptedPkcs8PrivateKey(empty, EDDsaPub, out bytesRead));
...
//                 Assert.ThrowsAny<CryptographicException>(
//                     () => key.ImportEncryptedPkcs8PrivateKey(empty, EDDsaPriv, out bytesRead));
...
//             using (EDDsa key = EDDsaFactory.Create())
//             {
//                 EDDsaParameters srcParameters = TestData.EDDsa2048Params;
//                 EDDsaParameters EDDsaParameters = new EDDsaParameters
//                 {
//                     Modulus = srcParameters.Modulus,
//                     Exponent = srcParameters.Exponent,
//                 };

//                 key.ImportParameters(EDDsaParameters);

//                 Assert.ThrowsAny<CryptographicException>(
//                     () => key.ExportEDDsaPrivateKey());

//                 Assert.ThrowsAny<CryptographicException>(
//                     () => key.TryExportEDDsaPrivateKey(Span<byte>.Empty, out _));
...
//         [Fact]
//         [ActiveIssue("https://github.com/dotnet/runtime/issues/62547", TestPlatforms.Android)]
//         public static void ReadWriteEDDsa2048EncryptedPkcs8_Pbes2HighIterations()
//         {
...
//         }
        ...
        //     in EDDsaParameters expected)
        // {
        //     ReadWriteKey(
        //         base64EncPkcs8,
        //         expected,
        //         (EDDsa EDDsa, ReadOnlySpan<byte> source, out int read) =>
        //             EDDsa.ImportEncryptedPkcs8PrivateKey(password, source, out read),
        //         EDDsa => EDDsa.ExportEncryptedPkcs8PrivateKey(password, pbeParameters),
        //         (EDDsa EDDsa, Span<byte> destination, out int written) =>
        //             EDDsa.TryExportEncryptedPkcs8PrivateKey(password, pbeParameters, destination, out written),
        //         isEncrypted: true);
        // }
        ...
        //         expected,
        //         (EDDsa EDDsa, ReadOnlySpan<byte> source, out int read) =>
        //             EDDsa.ImportEncryptedPkcs8PrivateKey(passwordBytes, source, out read),
        //         EDDsa => EDDsa.ExportEncryptedPkcs8PrivateKey(passwordBytes, pbeParameters),
        //         (EDDsa EDDsa, Span<byte> destination, out int written) =>
        //             EDDsa.TryExportEncryptedPkcs8PrivateKey(passwordBytes, pbeParameters, destination, out written),
        //         isEncrypted: true);
        // }
        ...
        //     in EDDsaParameters expected)
        // {
        //     EDDsaParameters expectedPublic = new EDDsaParameters
        //     {
        ...
        //         expectedPublic,
        //         (EDDsa EDDsa, ReadOnlySpan<byte> source, out int read) =>
        //             EDDsa.ImportEDDsaPublicKey(source, out read),
        //         EDDsa => EDDsa.ExportEDDsaPublicKey(),
        //         (EDDsa EDDsa, Span<byte> destination, out int written) =>
        //             EDDsa.TryExportEDDsaPublicKey(destination, out written));
        // }
        ...
        //         expectedPublic,
        //         (EDDsa EDDsa, ReadOnlySpan<byte> source, out int read) =>
        //             EDDsa.ImportSubjectPublicKeyInfo(source, out read),
        //         EDDsa => EDDsa.ExportSubjectPublicKeyInfo(),
        //         (EDDsa EDDsa, Span<byte> destination, out int written) =>
        //             EDDsa.TryExportSubjectPublicKeyInfo(destination, out written));
        // }
        ...
        //         expected,
        //         (EDDsa EDDsa, ReadOnlySpan<byte> source, out int read) =>
        //             EDDsa.ImportEDDsaPrivateKey(source, out read),
        //         EDDsa => EDDsa.ExportEDDsaPrivateKey(),
        //         (EDDsa EDDsa, Span<byte> destination, out int written) =>
        //             EDDsa.TryExportEDDsaPrivateKey(destination, out written));
        // }

        // private static void ReadWriteBase64Pkcs8(string base64Pkcs8, in EDDsaParameters expected)
        // {
        //     ReadWriteKey(
        //         base64Pkcs8,
        //         expected,
        //         (EDDsa EDDsa, ReadOnlySpan<byte> source, out int read) =>
        //             EDDsa.ImportPkcs8PrivateKey(source, out read),
        //         EDDsa => EDDsa.ExportPkcs8PrivateKey(),
        //         (EDDsa EDDsa, Span<byte> destination, out int written) =>
        //             EDDsa.TryExportPkcs8PrivateKey(destination, out written));
        // }
        ...
        //     Func<EDDsa, byte[]> writeArrayFunc,
        //     WriteKeyToSpanFunc writeSpanFunc,
        //     bool isEncrypted = false)
        // {
        //     bool isPrivateKey = expected.Key != null;
        ...
        //     using (EDDsa EDDsa = EDDsaFactory.Create())
        //     {
        //         readAction(EDDsa, derBytes, out int bytesRead);
        //         Assert.Equal(derBytes.Length, bytesRead);

        //         arrayExport = writeArrayFunc(EDDsa);

        //         EDDsaParameters EDDsaParameters = EDDsa.ExportParameters(isPrivateKey);
        //         EDDsaImportExportTests.AssertKeyEquals(expected, EDDsaParameters);
        //     }
        ...
        //     {
        //         Assert.ThrowsAny<CryptographicException>(
        //             () => readAction(EDDsa, arrayExport.AsSpan(1), out _));

        //         Assert.ThrowsAny<CryptographicException>(
        //             () => readAction(EDDsa, arrayExport.AsSpan(0, arrayExport.Length - 1), out _));

        //         readAction(EDDsa, arrayExport, out int bytesRead);
        ...
        //         Assert.False(
        //             writeSpanFunc(EDDsa, Span<byte>.Empty, out int bytesWritten),
        //             "Write to empty span");
        ...
        //         Assert.True(writeSpanFunc(EDDsa, tooBig.AsSpan(WriteShift), out bytesWritten));
        ...
        //     using (EDDsa EDDsa = EDDsaFactory.Create())
        //     {
        //         readAction(EDDsa, tooBig.AsSpan(WriteShift), out int bytesRead);
        //         Assert.Equal(arrayExport.Length, bytesRead);

        //         arrayExport.AsSpan().Fill(0xCA);

        //         Assert.True(
        //             writeSpanFunc(EDDsa, arrayExport, out int bytesWritten),
        ...
        private delegate void ReadKeyAction(EDDsa EDDsa, ReadOnlySpan<byte> source, out int bytesRead);
        private delegate bool WriteKeyToSpanFunc(EDDsa EDDsa, Span<byte> destination, out int bytesWritten);
    }
}
